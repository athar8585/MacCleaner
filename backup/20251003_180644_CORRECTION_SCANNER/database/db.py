...
def record_clean_run(categories_cleaned, space_freed_mb, duration_sec=0):
    """Enregistrer un nettoyage avec paramètres standards"""
    import datetime
    started_at = datetime.datetime.now().isoformat()
    with _LOCK, sqlite3.connect(DB_PATH) as conn:
        conn.execute(
            "INSERT INTO clean_runs (started_at, duration_sec, space_freed_mb, categories, mode) VALUES (?,?,?,?,?)",
            (started_at, duration_sec, space_freed_mb, categories_cleaned, 'real')
        )
        conn.commit()
